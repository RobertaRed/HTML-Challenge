...
function showMeme() {
  ...
  // quando clicco showmeme devo eliminare dalla pagina la risposta del riddle
  // selezionare elemento risposta
  // e poi eliminarlo se è presente
  
  
  
  
  
  const parentJoke= document.querySelector(".joke");
  
  
  const jokeprecedente= document.querySelector("p.broma");
  if (jokeprecedente){
    parentJoke.removeChild(jokeprecedente);
  }
   ...
   const parentQuote=
    ...
    const quoteprecedente= document.querySelector("p.citazione");
    if (quoteprecedente){parentQuote.removeChild(quoteprecedente);
  ...
  const parentRiddle=
    ...
    const riddleprecedente= document.querySelector("p.indovinello");
    if (riddleprecedente){parentRiddle.removeChild(riddleprecedente);
                          
                          
     
      
                          
     const eliminarisp= document.querySelector("#risposta"); 
                          
       if (eliminarisp) {
       const parentrisp= document.querySelector(".Answers");
         parentrisp.removeChild(eliminarisp);
  ...
  // 0 eliminare immagine precedente
  // 0a selezionare immagine precedente e salvare in variabile
  // 0b eliminare immagine
  
  
  const imgprecedente= document.querySelector("img");
  const parent = document.querySelector(".meme");
  if (imgprecedente){    parent.removeChild(imgprecedente);                   
  }
  
  
  
  
 // 1- trovare il padre e salvare variabile
  // 2 creare elemento immagine e salvare variabile
  // 3 aggiungere url all' immagine
  // 4 appendere il figlio al padre
  
  
  
  const imagine= document.createElement('img');
  imagine.src = randomMemeUrl;
  parent.appendChild(imagine);
...
function showJoke() {
  ...
  const randomJokeText = getRandomData('jokes');
   ...
   const parentMeme= document.querySelector(".meme");
  
  const memeprecedente= document.querySelector("img");
  if (memeprecedente){
    parentMeme.removeChild(memeprecedente);
  }
  
  const parentQuote= document.querySelector(".quote");
  
  const quoteprecedente= document.querySelector(".citazione");
  if (quoteprecedente){
    parentQuote.removeChild(quoteprecedente);
  }
  
  const parentRiddle= document.querySelector(".riddle");
  
  const riddleprecedente= document.querySelector("p.indovinello");
  if (riddleprecedente){
    parentRiddle.removeChild(riddleprecedente);
  }
  ...
  // trovare padre e salvare variabile
  // creare paragrafo e variabile
  //aggiungere testo al paragrafo
  //appendere il figlio al padre
  const parent = document.querySelector(".joke");
 const jokeprecedente= document.querySelector('p.broma'); 
  if (jokeprecedente) { 
  parent.removeChild(jokeprecedente); }
  ...
  joke1.setAttribute( 'class', 'broma' );
  
  
const text = document.createTextNode(randomJokeText);

joke1.appendChild(text);
...
function showQuote() {
 ...
 const quoteprecedente= document.querySelector('p.citazione'); 
  if (quoteprecedente) { 
  parent.removeChild(quoteprecedente);
  }


 const quote1= document.createElement('p');
  quote1.setAttribute( 'class', 'citazione' );
const text = 
 document.createTextNode(randomQuote.quote);     


  
quote1.appendChild(text);
  
 parent.appendChild(quote1);
...
function showRiddle() {
 ...
 const quoteprecedente= document.querySelector('p.citazione'); 
  ...
  // 0 eliminare riddle precedente
  // 0a selezionare riddle precedente e salvare in variabile
  // 0b eliminare riddle
  const parent = document.querySelector(".riddle");
 const riddleprecedente= document.querySelector('p.indovinello'); 
  if (riddleprecedente) { 
  parent.removeChild(riddleprecedente);
  }
  ...
  const riddle1= document.createElement('p');
  riddle1.setAttribute( 'class', 'indovinello');
  
  console.log(randomRiddle)
    const text = document.createTextNode(randomRiddle.question);         
    
    riddle1.appendChild(text);
  
 parent.appendChild(riddle1); 
  
  
  const parent1 = document.querySelector(".Answers");
  
 const answer1= document.createElement('p');
  answer1.setAttribute( 'id', 'risposta');
  
  answer1.hidden = true;
  
    const text1 = 
document.createTextNode(randomRiddle.answer); 
  
  
     answer1.appendChild(text1);
  
 parent1.appendChild(answer1);    
...
function revealAnswers() {
  // rendere visibile riddle answer quando riddle me è visibile
  // controllare se c'è riddle me in pagina
  // devo usare un if
  // se è vero :seleziona la risposta, rivela risposta
  // se è falso : non fare niente
  
  const riddle = document.querySelector(".indovinello");
